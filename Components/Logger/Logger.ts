...
const outputsDir = path.join(
  process.env.LOGS_DIRECTORY || process.cwd(),
  "../../../user-data/Logs/"
);
  ...
  return winston.createLogger({
    ...
    transports: [
      ...
      new winston.transports.File({
        filename: path.join(outputsDir, "Errors.log"),
        level: "error",
      }),
      new winston.transports.File({
        filename: path.join(outputsDir, "Warnings.log"),
        level: "warn",
      }),
      new winston.transports.File({
        filename: path.join(outputsDir, "Infos.log"),
        level: "info",
      }),
      new winston.transports.File({
        filename: path.join(outputsDir, "Debug.log"),
        level: "debug",
      }),
      new winston.transports.File({
        filename: path.join(outputsDir, "Combined.log"),
