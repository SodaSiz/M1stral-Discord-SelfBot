...
import blagues_lists from '../../Utils/Misc/blagues_api_lists.json';
import { prefix } from '../../Utils/Misc/Settings.json';
import { ClientAttributes } from '../../Types/Client';
import { Message } from 'discord.js-selfbot-v13';
import { Categories } from 'blagues-api/dist/types/types';
...
export default {
    ...
    run: async (client: ClientAttributes, message: Message, args: string[]) => {
        ...
        try {
            ...
            if (args.length > 0) {
                if (args[0].toLowerCase() === 'categories') {
                    ...
                    if (categories.length > 0) {
                        ...
                        return message.channel.send(`Voici la liste des catégories disponibles : ${categoriesList}`);
                    } else {
                        return message.channel.send("Il n'y a pas de catégories disponibles.");
                    }
                } else {
                    ...
                    if (!blagues_lists.categories.includes(category)) {
                        return message.channel.send(`**La catégorie de blagues ${args[0]} n'existe pas !**\nPour obtenir la liste des catégories disponibles, vous pouvez entrer ${prefix}blagues categories.`);
                    }

                    // Accéder à la blague de la catégorie spécifiée
                    blague = await blagues.randomCategorized(blagues.categories[category as Categories]);
                }
            } else {
            ...
            return message.channel.send("Une erreur s'est produite lors de la récupération de la blague.");
        }
    }
}
