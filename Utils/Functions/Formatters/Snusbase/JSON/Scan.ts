        ...
        userArray.forEach((user) => {
          for (const field in fieldConfig) {
            if (user[field] !== undefined && user[field] !== null) {
              let value = user[field] as string;
              const normalizedValue = value.toLowerCase(); // Normaliser la casse

              if (fieldConfig[field].showSource) {
                value = `${value} (\`${key}\`)`; // Ajouter la source entre parenthèses
              }

              if (!extractedInfo[field].has(normalizedValue)) {
                extractedInfo[field].set(normalizedValue, [
                  { value, sources: [key] },
                ]);
              } else {
